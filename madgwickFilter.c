...
LOG_MODULE_REGISTER(madgwickFilter, CONFIG_SLM_LOG_LEVEL);
...
void filter_test(void)
{
    ...
    for(float i = 0; i<1000; i++){
        // acelerometer값(가속도) 3개 + gyro값(각속도) 3개로 복소평면상의 좌표를 계산한다
        // 동일한 값이 입력되더라도 이전의 가속도, 각속도값에 누적이 되어 최종값을 계산한다
        // 따라서 for loop로 여러번 돌리면, 돌린만큼 누적이 된 결과값이 보여진다
        imu_filter(0.05, 0.05, 0.9, 0, 0, 0);
        // 복소평면상의 좌표를 오일러 각도로 변환한다
        eulerAngles(q_est, &roll, &pitch, &yaw);
        LOG_INF("Time (s): %.3f, roll: %f, pitch: %f, yaw: %f\n", i * DELTA_T, roll, pitch, yaw);
    }
